...
#### 6.4.3 สร้างไฟล์ routes/users.js
...
#### 6.4.4 สร้างไฟล์ controllers/userController.js
...
#### 6.4.5 สร้างไฟล์ routes/devices.js
...
#### 6.4.6 สร้างไฟล์ controllers/deviceController.js
...
#### 10.6.3.2 สร้าง Postman Collection
...
```json
{
  "info": {
    "name": "IoT System API",
    "description": "API Collection for IoT System with MQTT, Node-RED, ESP32",
    "schema": "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"
  },
  "variable": [
    {
      "key": "base_url",
      "value": "http://192.168.10.10:3000/api",
      "type": "string"
    },
    {
      "key": "token",
...
#### 10.6.3.3 วิธี Import Postman Collection

1. **เปิด Postman**
2. **คลิก Import:**
   - คลิก "Import" ที่มุมซ้ายบน
   - เลือกไฟล์ `IoT-System-API.postman_collection.json`
   - หรือ Drag & Drop ไฟล์
3. **ตั้งค่า Environment Variables:**
   - คลิก "Environments" > "Create Environment"
   - ตั้งชื่อ: "IoT System Production"
   - เพิ่ม Variables:
     - `base_url`: `http://192.168.10.10:3000/api`
     - `token`: (จะถูกตั้งค่าอัตโนมัติหลัง Login)

#### 10.6.3.4 ตัวอย่างการทดสอบ API
...
#### 10.6.3.5 ตั้งค่า Automated Tests
...
// Test สำหรับ Create Sensor Data
pm.test("Status code is 201", function () {
    pm.response.to.have.status(201);
});

pm.test("Sensor data created successfully", function () {
    var jsonData = pm.response.json();
    pm.expect(jsonData.status).to.eql("success");
...
#### 10.6.3.6 วิธีใช้งาน Postman Collection
...
2. **ตั้งค่า Environment:**
   - สร้าง Environment ชื่อ "IoT System Production"
   - ตั้งค่า `base_url`: `http://192.168.10.10:3000/api`

3. **ทดสอบ API:**
   - เริ่มจาก "Login" เพื่อรับ Token
   - Token จะถูกเก็บอัตโนมัติใน Collection Variable
   - ทดสอบ API อื่นๆ โดยใช้ Token ที่ได้

4. **Run Collection:**
   - คลิก "Run" ที่ Collection
   - เลือก Requests ที่ต้องการทดสอบ
   - คลิก "Run IoT System API"
   - ดูผลลัพธ์ใน Test Results

#### 10.6.3.7 สร้างไฟล์ Postman Collection

**สร้างไฟล์ `postman/IoT-System-API.postman_collection.json`:**

```bash
# สร้างโฟลเดอร์
mkdir -p D:\Skill-PWS\postman

# สร้างไฟล์ Collection (Copy เนื้อหาจากด้านบน)
# หรือใช้ Postman Export Collection
```

**Export Collection จาก Postman:**
1. เปิด Postman
2. คลิกขวาที่ Collection > "Export"
3. เลือก "Collection v2.1"
4. บันทึกเป็น `IoT-System-API.postman_collection.json`
...
## 11. สรุปข้อมูลสำคัญ (Quick Reference)

### 11.1 User Accounts และ Passwords

#### Application Users (IoT System)
| Username | Password | Email | Role | หมายเหตุ |
|----------|----------|-------|------|----------|
| `admin` | `1234` | admin@sisat.lan | admin | Full access |
| `user1` | `1234` | user1@sisat.lan | user_level_1 | Read/Write access |
| `user2` | `1234` | user2@sisat.lan | user_level_2 | Read-only access |

**หมายเหตุ:** ทุก user ใช้ password เดียวกัน: `1234`

#### System Users (Linux VMs)
| Hostname | Username | Password | IP Address |
|----------|----------|----------|------------|
| git.sisat.lan | `it` | `12345678` | 192.168.10.20 |
| prod.sisat.lan | `it` | `12345678` | 192.168.10.10 |
| proxmox.sisat.lan | `root` | (ตั้งค่าเอง) | 192.168.10.5 |
...
|----------|----------|----------|
| `mqtt_user` | `mqtt_password` | สำหรับ MQTT Broker (Mosquitto) |

#### Network Device Default Passwords
| Device | Default Username | Default Password | หมายเหตุ |
|--------|------------------|------------------|----------|
| Router TP-LINK | `admin` | `admin` | เปลี่ยนหลังติดตั้ง |
| Switch HP | `admin` | (ว่างเปล่า) | เปลี่ยนหลังติดตั้ง |
| GitLab | `root` | (จาก initial_root_password) | เปลี่ยนหลังติดตั้ง |

#### Wireless Network
| SSID | Password | Security | หมายเหตุ |
|------|----------|----------|----------|
| `@iot` | `12345678` | WPA2-PSK | Router หลัก |
| `@wifi` | `12345678` | WPA2-PSK | Access Point (Router ตัวที่ 2) |

---

### 11.2 IP Addresses และ Network Configuration

#### Network Infrastructure
| Device/Service | IP Address | Subnet | Gateway | VLAN |
|---------------|------------|--------|--------|------|
| Router TP-LINK (LAN) | 192.168.10.1 | 255.255.255.0 | - | - |
| Router TP-LINK (Default) | 192.168.1.1 | - | - | - |
| Access Point | 192.168.10.3 | 255.255.255.0 | 192.168.10.1 | VLAN 10 |
| Proxmox Host | 192.168.10.5 | 255.255.255.0 | 192.168.10.1 | VLAN 10 |
| DHCP Range | 192.168.10.30-50 | 255.255.255.0 | 192.168.10.1 | VLAN 10 |

#### Virtual Machines
| VM Name | Hostname | IP Address | Subnet | Gateway | VLAN |
|---------|----------|------------|--------|---------|------|
| Git Server | git.sisat.lan | 192.168.10.20 | /24 | 192.168.10.1 | VLAN 10 |
| Production Server | prod.sisat.lan | 192.168.10.10 | /24 | 192.168.10.1 | VLAN 10 |

#### Docker Containers (Internal Network)
| Container | Service Name | Internal IP | Port Mapping | Network |
|-----------|--------------|-------------|--------------|---------|
| Backend | `backend` | (auto) | 3000:3000 | iot-network |
| Frontend | `frontend` | (auto) | 3001:3001 | iot-network |
| Database | `db` | (auto) | 3306:3306 | iot-network |
| MQTT Broker | `mqtt` | (auto) | 1883:1883, 9001:9001 | iot-network |
| Node-RED | `nodered` | (auto) | 1880:1880 | iot-network |
| phpMyAdmin | `phpmyadmin` | (auto) | 8080:80 | iot-network |

**หมายเหตุ:** Docker containers ใช้ service name สำหรับ internal communication (เช่น `db`, `mqtt`, `backend`)
...
#### Web Interfaces
| Service | URL | Port | Protocol | Authentication |
|---------|-----|------|----------|----------------|
| Proxmox Web UI | https://192.168.10.5:8006 | 8006 | HTTPS | root / (password) |
| GitLab | http://192.168.10.20 | 80 | HTTP | root / (initial_root_password) |
| Frontend Application | http://192.168.10.10:3001 | 3001 | HTTP | Application login |
| Node-RED | http://192.168.10.10:1880 | 1880 | HTTP | (ตั้งค่าเอง) |
| phpMyAdmin | http://192.168.10.10:8080 | 8080 | HTTP | root / rootpassword123 |

#### API Endpoints
| Service | Base URL | Port | Protocol | Authentication |
|---------|----------|------|----------|----------------|
| Backend API | http://192.168.10.10:3000/api | 3000 | HTTP | JWT Token |
| Health Check | http://192.168.10.10:3000/api/health | 3000 | HTTP | - |
...
#### MQTT
| Service | Host | Port | Protocol | Authentication |
|---------|------|------|----------|----------------|
| MQTT Broker | 192.168.10.10 | 1883 | MQTT | mqtt_user / mqtt_password |
| MQTT WebSocket | 192.168.10.10 | 9001 | WebSocket | mqtt_user / mqtt_password |

**MQTT Topics:**
- `sensor/temp/#` - Temperature sensor data
- `sensor/humidity/#` - Humidity sensor data
- `command/device001/#` - Device control commands

#### Database
| Service | Host | Port | Protocol | Authentication |
|---------|------|------|----------|----------------|
| MariaDB | 192.168.10.10 | 3306 | MySQL | iot_user / iot_password_123 |
| MariaDB (Root) | 192.168.10.10 | 3306 | MySQL | root / rootpassword123 |

#### Development URLs (Local)
| Service | URL | Port | หมายเหตุ |
|---------|-----|------|----------|
| Frontend Dev | http://localhost:3001 | 3001 | Development mode |
| Backend Dev | http://localhost:3000 | 3000 | Development mode |
| API Dev | http://localhost:3000/api | 3000 | Development mode |
...
|------|---------|----------|--------|
| 80 | GitLab | HTTP | Internal |
| 443 | GitLab (HTTPS) | HTTPS | Internal |
| 1880 | Node-RED | HTTP | Internal |
| 1883 | MQTT Broker | MQTT | Internal |
| 3000 | Backend API | HTTP | Internal |
| 3001 | Frontend | HTTP | Internal |
| 3306 | MariaDB | MySQL | Internal |
| 8006 | Proxmox | HTTPS | Internal |
| 8080 | phpMyAdmin | HTTP | Internal |
| 9001 | MQTT WebSocket | WebSocket | Internal |

**หมายเหตุ:** Ports ทั้งหมดเป็น Internal access เท่านั้น (ภายใน network 192.168.10.0/24)

---

### 11.5 Quick Access Commands
...
```bash
# Connect to MariaDB
docker exec -it mariadb mysql -u iot_user -piot_password_123 iot_db

# Connect as root
docker exec -it mariadb mysql -u root -prootpassword123
```
...
# Login
curl -X POST http://192.168.10.10:3000/api/auth/login \
...
### 11.6 Environment Variables Summary
...
```typescript
apiBase: 'http://192.168.10.10:3000/api'
```
