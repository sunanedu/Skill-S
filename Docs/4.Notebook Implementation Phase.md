...
- **Network**: Host-Only Network (172.16.10.0/24)
- **VMs**:
...
### 2.2 สร้าง VirtualBox Host-Only Network

**หมายเหตุสำคัญ:** Internal Network ใน VirtualBox จะทำให้ VMs เชื่อมต่อกันได้ แต่ Windows Host จะไม่สามารถเชื่อมต่อกับ VMs ได้โดยตรง

**วิธีแก้:** ใช้ **Host-Only Adapter** แทน Internal Network เพื่อให้ Windows Host เชื่อมต่อกับ VMs ได้

**การตั้งค่า Network Adapter:**
- **Adapter 1 (Host-Only)**: สำหรับเชื่อมต่อระหว่าง Windows Host กับ VMs (IP: 172.16.10.x)
- **Adapter 2 (NAT)**: สำหรับออกอินเทอร์เน็ตผ่าน Windows Host (จำเป็นสำหรับติดตั้งโปรแกรม)

#### 2.2.1 สร้าง Host-Only Network

**หมายเหตุ:** VirtualBox มี DHCP Server สำหรับ Host-Only Network เหมือน VMware แต่ในกรณีนี้เราจะใช้ Static IP เพื่อควบคุม IP Address ให้แน่นอน

**ขั้นตอนการสร้าง Host-Only Network:**

**วิธีที่ 1: ใช้ Static IP (แนะนำ - ใช้ในคู่มือนี้)**

1. **เปิด VirtualBox Manager**
2. **ไปที่ File > Host Network Manager** (หรือ Tools > Network > Host Network Manager)
3. **คลิก "Create"** เพื่อสร้าง Host-Only Network ใหม่
4. **ตั้งค่า Host-Only Network:**
   - **Adapter Tab:**
     - IPv4 Address: `172.16.10.1` (Windows Host จะใช้ IP นี้)
     - IPv4 Network Mask: `255.255.255.0`
     - ✅ Enable DHCP: **ไม่ต้องเลือก** (เราจะใช้ Static IP)
   - **DHCP Server Tab:**
     - ✅ Enable Server: **ไม่ต้องเลือก**
5. **คลิก "Apply"** และ "OK"

**วิธีที่ 2: ใช้ DHCP (ทางเลือก - เหมือน VMware)**
...
4. **ตั้งค่า Host-Only Network:**
   - **Adapter Tab:**
     - IPv4 Address: `172.16.10.1` (Windows Host จะใช้ IP นี้)
     - IPv4 Network Mask: `255.255.255.0`
     - ✅ Enable DHCP: **เลือก** (เปิดใช้งาน DHCP)
   - **DHCP Server Tab:**
     - ✅ Enable Server: **เลือก**
     - Server Address: `172.16.10.2` (IP ของ DHCP Server)
     - Server Mask: `255.255.255.0`
     - Lower Address Bound: `172.16.10.100` (IP Range เริ่มต้น)
     - Upper Address Bound: `172.16.10.200` (IP Range สิ้นสุด)
5. **คลิก "Apply"** และ "OK"

**เปรียบเทียบ Static IP vs DHCP:**

| วิธี | ข้อดี | ข้อเสีย |
|------|-------|---------|
| **Static IP** | - ควบคุม IP Address ได้แน่นอน<br>- ไม่ต้องกังวลเรื่อง IP เปลี่ยน<br>- ตั้งค่า Host File ได้ง่าย | - ต้องตั้งค่าเองบน VM<br>- ต้องจำ IP Address |
| **DHCP** | - ตั้งค่าอัตโนมัติ<br>- เหมือน VMware<br>- ไม่ต้องจำ IP | - IP อาจเปลี่ยนได้<br>- ต้องเช็ค IP ทุกครั้ง<br>- ต้องใช้ `ip addr show` เพื่อดู IP |

**ตรวจสอบ Host-Only Network:**
```bash
# บน Windows PowerShell (Run as Administrator)
ipconfig

# ควรเห็น VirtualBox Host-Only Network Adapter
# IPv4 Address: 172.16.10.1
```

**หมายเหตุ:** 
- ถ้าใช้ **Static IP**: VM จะต้องตั้งค่า IP เอง (ตามขั้นตอนใน Section 2.4)
- ถ้าใช้ **DHCP**: VM จะได้รับ IP อัตโนมัติจาก VirtualBox DHCP Server (ไม่ต้องตั้งค่าเอง)

---

### 2.3 สร้าง Virtual Machines

#### 2.3.1 สร้าง VM1: Git Server
...
3. **ตั้งค่า Network:**
   ...
   - **Adapter 1** (สำหรับเชื่อมต่อกับ Windows Host):
     - Enable Network Adapter: ✓
     - Attached to: **Host-Only Adapter**
     - Name: `VirtualBox Host-Only Ethernet Adapter` (หรือชื่อที่สร้างไว้)
   - **Adapter 2** (สำหรับออกอินเทอร์เน็ต - จำเป็น):
     - Enable Network Adapter: ✓
     - Attached to: **NAT**
     - **หมายเหตุ:** NAT จะทำให้ VM สามารถออกอินเทอร์เน็ตได้ผ่าน Windows Host
...
4. **ติดตั้ง Ubuntu:**
   ...
   - ตั้งค่า:
     ...
     - Gateway: `172.16.10.1` (IP ของ Windows Host ใน Host-Only Network)
...
# ทดสอบ Ping จาก VM ไปยัง Windows Host
ping 172.16.10.1

# ทดสอบ Ping จาก Windows Host ไปยัง VM
...
#### 2.3.2 สร้าง VM2: Production Server
...
3. **ติดตั้ง Ubuntu:**
   ...
   - ตั้งค่า:
     ...
     - **Network Configuration:**
       - **Interface 1 (enp0s3 - Host-Only)**: Static IP `172.16.10.10/24`, Gateway `172.16.10.1`
       - **Interface 2 (enp0s8 - NAT)**: DHCP (จะได้รับ IP อัตโนมัติจาก VirtualBox NAT)
     - DNS: `8.8.8.8` (ใช้ DNS จาก NAT Adapter)
...
### 2.4 ตั้งค่า Network Configuration บน VMs

**หมายเหตุ:** VM จะมี 2 Network Interfaces:
- **enp0s3**: Host-Only Adapter (สำหรับเชื่อมต่อกับ Windows Host) - ใช้ Static IP
- **enp0s8**: NAT Adapter (สำหรับออกอินเทอร์เน็ต) - ใช้ DHCP

#### 2.4.1 ตั้งค่า Network บน Git Server

**วิธีที่ 1: ใช้ Static IP (แนะนำ)**

**คำสั่งตั้งค่า:**

```bash
# SSH เข้า Git Server
# ถ้าใช้ DHCP: ต้องเช็ค IP ก่อนด้วย ip addr show
ssh it@172.16.10.20
...
**เนื้อหาไฟล์ (Static IP):**
```yaml
network:
  version: 2
  ethernets:
    # Host-Only Adapter (สำหรับเชื่อมต่อกับ Windows Host) - Static IP
    enp0s3:
      dhcp4: false
      addresses:
        - 172.16.10.20/24
      ...
      nameservers:
        addresses:
          - 8.8.8.8
          - 8.8.4.4
    # NAT Adapter (สำหรับออกอินเทอร์เน็ต) - DHCP
    enp0s8:
      dhcp4: true
      # NAT จะจัดการ Gateway และ DNS อัตโนมัติ
```

**วิธีที่ 2: ใช้ DHCP (ทางเลือก)**
...
network:
  version: 2
  ethernets:
    # Host-Only Adapter (สำหรับเชื่อมต่อกับ Windows Host) - DHCP
    enp0s3:
      dhcp4: true
      # VirtualBox DHCP Server จะให้ IP อัตโนมัติ (172.16.10.100-200)
    # NAT Adapter (สำหรับออกอินเทอร์เน็ต) - DHCP
    enp0s8:
      dhcp4: true
      # NAT จะจัดการ Gateway และ DNS อัตโนมัติ
...
# ถ้าใช้ DHCP: เช็ค IP ที่ได้รับ
# enp0s3: 172.16.10.x (จาก VirtualBox DHCP)
# enp0s8: 10.0.2.x (จาก VirtualBox NAT)
```

**หมายเหตุ:** 
- ถ้าใช้ DHCP: IP Address อาจเปลี่ยนได้ ต้องเช็คด้วย `ip addr show` ทุกครั้ง
- ถ้าใช้ Static IP: IP Address จะคงที่ (172.16.10.20)

---

#### 2.4.2 ตั้งค่า Network บน Production Server
...
**หมายเหตุ:** 
- ถ้าใช้ DHCP: IP Address อาจเปลี่ยนได้ ต้องเช็คด้วย `ip addr show` ทุกครั้ง
- ถ้าใช้ Static IP: IP Address จะคงที่ (172.16.10.10)

---

### 2.5 ตั้งค่า Host File บน Windows
...
### 2.6 ทดสอบการเชื่อมต่อระหว่าง Windows Host กับ VMs

**ทดสอบจาก Windows Host:**

```powershell
# บน Windows PowerShell

# 1. ตรวจสอบ VirtualBox Host-Only Network Adapter
ipconfig
# ควรเห็น: VirtualBox Host-Only Ethernet Adapter
#          IPv4 Address: 172.16.10.1

# 2. ทดสอบ Ping ไปยัง VMs
ping 172.16.10.20  # Git Server
ping 172.16.10.10  # Production Server

# 3. ทดสอบ SSH
ssh it@172.16.10.20
ssh it@172.16.10.10

# 4. ทดสอบการเชื่อมต่อ Database (ถ้า Database ทำงานแล้ว)
# (ต้องติดตั้ง MySQL Client หรือใช้ MySQL Workbench)
mysql -h 172.16.10.10 -u iot_user -piot_password_123 iot_db
```
...
```bash
# SSH เข้า VM
ssh it@172.16.10.20  # หรือ 172.16.10.10

# 1. ตรวจสอบ Network Interface
ip addr show
# ควรเห็น:
# - enp0s3: 172.16.10.x (Host-Only Adapter)
# - enp0s8: 10.0.2.x (NAT Adapter - IP จาก VirtualBox NAT)

# 2. ทดสอบ Ping ไปยัง Windows Host (ผ่าน Host-Only)
ping 172.16.10.1

# 3. ทดสอบ Ping ไปยัง VM อื่น (ผ่าน Host-Only)
ping 172.16.10.20  # จาก Production Server
ping 172.16.10.10  # จาก Git Server

# 4. ทดสอบการออกอินเทอร์เน็ต (ผ่าน NAT)
ping 8.8.8.8
...
# 6. ทดสอบ apt update (ต้องใช้ NAT)
sudo apt update
```
...
```powershell
# ถ้า Ping ไม่ได้ ให้ตรวจสอบ:

# 1. ตรวจสอบว่า VirtualBox Host-Only Adapter ทำงานอยู่
# Control Panel > Network and Sharing Center > Change adapter settings
# ควรเห็น "VirtualBox Host-Only Network" และ Status = Enabled

# 2. ตรวจสอบ Firewall บน Windows
# Windows Defender Firewall อาจบล็อกการเชื่อมต่อ
# เปิด Windows Defender Firewall > Advanced Settings
# เพิ่ม Inbound Rule สำหรับ Port 22 (SSH), 3306 (MySQL), 1883 (MQTT)

# 3. Restart VirtualBox Host-Only Adapter
# Control Panel > Network and Sharing Center > Change adapter settings
# คลิกขวาที่ "VirtualBox Host-Only Network" > Disable
# คลิกขวา > Enable

# 4. Restart VMs
# ปิด VM แล้วเปิดใหม่

# 5. ถ้า VM ออกอินเทอร์เน็ตไม่ได้ (NAT ไม่ทำงาน)
# - ตรวจสอบว่า Adapter 2 ถูก Enable และตั้งเป็น NAT
# - ตรวจสอบว่า Windows Host มีการเชื่อมต่ออินเทอร์เน็ต
# - ทดสอบ: ping 8.8.8.8 จาก VM
```

### 2.7 สรุปการตั้งค่า Network

**Network Architecture:**

```
Windows Host (172.16.10.1)
│
├─ Host-Only Network (172.16.10.0/24)
│  ├─ Git Server VM (172.16.10.20) - enp0s3
│  └─ Production Server VM (172.16.10.10) - enp0s3
│
└─ NAT Network (10.0.2.0/24 - VirtualBox Internal)
   ├─ Git Server VM (10.0.2.x) - enp0s8 → Internet
   └─ Production Server VM (10.0.2.x) - enp0s8 → Internet
```

**การใช้งาน:**
- **Host-Only (enp0s3)**: ใช้สำหรับเชื่อมต่อระหว่าง Windows Host กับ VMs (SSH, Database, MQTT)
- **NAT (enp0s8)**: ใช้สำหรับออกอินเทอร์เน็ต (ติดตั้งโปรแกรม, apt update, download packages)

**ข้อดี:**
- ✅ Windows Host เชื่อมต่อกับ VMs ได้ (ผ่าน Host-Only)
- ✅ VMs ออกอินเทอร์เน็ตได้ (ผ่าน NAT)
- ✅ VMs เชื่อมต่อกันได้ (ผ่าน Host-Only)
- ✅ ไม่ต้องพึ่ง Physical Network
...
### 5.0 Development Mode Architecture

**เมื่อรันแบบ Development บน Windows 10 (D:\Skill-PWS\):**

**Services ที่รันบน Windows Host:**
- ✅ **Backend API**: `localhost:3000` (รันด้วย `npm run dev`)
- ✅ **Frontend Application**: `localhost:3001` (รันด้วย `npm run dev`)

**Services ที่รันบน Production Server VM (172.16.10.10):**
- ✅ **Database (MariaDB)**: `172.16.10.10:3306` (Docker Container: `iot-db`)
- ✅ **MQTT Broker**: `172.16.10.10:1883` (Docker Container: `iot-mqtt`)
- ✅ **Node-RED**: `172.16.10.10:1880` (Docker Container: `iot-nodered`)
- ✅ **phpMyAdmin**: `172.16.10.10:8080` (Docker Container: `iot-phpmyadmin`)

**การเชื่อมต่อ:**
```
Windows Host (D:\Skill-PWS\)
├─ Backend (localhost:3000)
│  └─ เชื่อมต่อ Database → 172.16.10.10:3306
│
├─ Frontend (localhost:3001)
│  └─ เรียก API → localhost:3000/api
│
└─ ESP32 Simulator
   └─ ส่งข้อมูล MQTT → 172.16.10.10:1883
      └─ Node-RED รับข้อมูล → บันทึกลง Database (172.16.10.10:3306)
```

**หมายเหตุ:**
- Backend และ Frontend รันบน Windows Host โดยตรง (ไม่ใช้ Docker) เพื่อความง่ายในการพัฒนา
- Database, MQTT, Node-RED รันบน Production Server VM ใน VirtualBox
- ต้องแน่ใจว่า Production Server VM ทำงานอยู่และสามารถเข้าถึงได้จาก Windows Host
...
#### 6.2.3 สร้าง Middleware สำหรับ Authentication
  ...
  <div class="min-h-screen bg-gray-100">
    ...
    <header class="bg-white shadow">
      <div class="container mx-auto px-4 py-4 flex justify-between items-center">
        <h1 class="text-2xl font-bold">IoT System Dashboard</h1>
    ...
    <main class="container mx-auto px-4 py-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="bg-white p-6 rounded-lg shadow">
          ...
          <p class="text-3xl font-bold text-blue-600">{{ latestTemp }}°C</p>
          <p class="text-sm text-gray-500 mt-2">Latest reading</p>
        </div>
        
        <div class="bg-white p-6 rounded-lg shadow">
          <h2 class="text-xl font-semibold mb-2 text-gray-700">Humidity</h2>
          <p class="text-3xl font-bold text-green-600">{{ latestHumidity }}%</p>
          <p class="text-sm text-gray-500 mt-2">Latest reading</p>
        </div>
        
        <div class="bg-white p-6 rounded-lg shadow">
          <h2 class="text-xl font-semibold mb-2 text-gray-700">Status</h2>
      ...
      <div class="bg-white p-6 rounded-lg shadow">
        ...
        <div v-if="loading && sensorData.length === 0" class="text-center py-8">
          <p class="text-gray-500">Loading sensor data...</p>
        </div>
        
        <div v-else-if="sensorData.length === 0" class="text-center py-8">
        ...
        <table v-else class="w-full">
          <thead>
            <tr class="border-b">
              ...
              <th class="text-left p-3 text-gray-700">Value</th>
              <th class="text-left p-3 text-gray-700">Topic</th>
              <th class="text-left p-3 text-gray-700">Timestamp</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              ...
              <td class="p-3 font-semibold">{{ item.sensor_value }}</td>
              <td class="p-3 text-sm text-gray-600">{{ item.mqtt_topic }}</td>
              <td class="p-3 text-sm text-gray-600">{{ formatDate(item.timestamp) }}</td>
            </tr>
          </tbody>
        </table>
...
const loadSensorData = async () => {
  ...
  try {
    ...
    const response = await axios.get(`${config.public.apiBase}/sensor-data`, {
      headers: {
...
#### 10.1.7 ทดสอบ API ด้วย Postman

**Postman** เป็นเครื่องมือสำหรับทดสอบ API ที่มี GUI ใช้งานง่าย รองรับการสร้าง Collections, Environments, และ Automated Tests

##### 10.1.7.1 ติดตั้ง Postman

```bash
# Windows: Download จาก https://www.postman.com/downloads/
# หรือใช้ Chocolatey
choco install postman

# macOS: Download จาก https://www.postman.com/downloads/
# หรือใช้ Homebrew
brew install --cask postman

# Linux: Download จาก https://www.postman.com/downloads/
# หรือใช้ Snap
snap install postman
```

##### 10.1.7.2 สร้าง Postman Collection

**สร้างไฟล์ `postman/IoT-System-API.postman_collection.json`:**
...
**สร้างไฟล์ `IoT-System-API.postman_collection.json`:**

```json
{
  "info": {
    "name": "IoT System API",
    "description": "API Collection for IoT System with MQTT, Node-RED, ESP32 Simulation",
      ...
      "item": [
        {
          ...
          "request": {
            ...
            "url": {
              "raw": "{{base_url}}/sensor-data?page=1&limit=10",
              "host": ["{{base_url}}"],
              "path": ["sensor-data"],
              "query": [
                {
...
##### 10.1.7.3 วิธี Import Postman Collection

1. **เปิด Postman**
2. **คลิก Import:**
   - คลิก "Import" ที่มุมซ้ายบน
   - เลือกไฟล์ `D:\Skill-PWS\postman\IoT-System-API.postman_collection.json`
   - หรือ Drag & Drop ไฟล์
3. **ตั้งค่า Collection Variables:**
   - คลิกขวาที่ Collection > "Edit"
   - ไปที่ Tab "Variables"
   - ตรวจสอบว่า `base_url` = `http://localhost:3000/api`
   - `token` จะถูกตั้งค่าอัตโนมัติหลัง Login

##### 10.1.7.4 ตัวอย่างการทดสอบ API
...
**หมายเหตุ:** Token จะถูกเก็บอัตโนมัติใน Collection Variable หลัง Login สำเร็จ

**3. ทดสอบ Get Sensor Data:**

```
GET {{base_url}}/sensor-data?page=1&limit=10
Authorization: Bearer {{token}}
```
...
**4. ทดสอบ Create Sensor Data:**

```
POST {{base_url}}/sensor-data
Authorization: Bearer {{token}}
Content-Type: application/json
...
##### 10.1.7.5 ตั้งค่า Automated Tests

**เพิ่ม Tests ใน Postman (ใน Tab "Tests" ของแต่ละ Request):**
...
pm.test("Response has token", function () {
    ...
    // เก็บ Token ใน Collection Variable
...
##### 10.1.7.6 วิธีใช้งาน Postman Collection

1. **Import Collection:**
   - เปิด Postman
   - คลิก "Import" > เลือกไฟล์ `IoT-System-API.postman_collection.json`

2. **ทดสอบ API:**
   - เริ่มจาก "Health Check" เพื่อตรวจสอบว่า Backend ทำงาน
   - ต่อด้วย "Login" เพื่อรับ Token (Token จะถูกเก็บอัตโนมัติ)
   - ทดสอบ API อื่นๆ โดยใช้ Token ที่ได้

3. **Run Collection:**
   - คลิกขวาที่ Collection > "Run collection"
   - เลือก Requests ที่ต้องการทดสอบ
   - คลิก "Run IoT System API"
   - ดูผลลัพธ์ใน Test Results
...
- Gateway: 172.16.10.1 (Windows Host IP)
...
**Backend (.env) - รันบน Windows Host:**
```env
NODE_ENV=development
PORT=3000
JWT_SECRET=your-secret-key-change-this-in-production
...
# Database - เชื่อมต่อไปยัง Production Server VM
DB_HOST=172.16.10.10
DB_PORT=3306
DB_USER=iot_user
DB_PASSWORD=iot_password_123
...
**Frontend (nuxt.config.ts) - รันบน Windows Host:**
```typescript
// API Base URL - เรียก Backend ที่รันบน localhost
apiBase: 'http://localhost:3000/api'
```

**ESP32 Simulator - รันบน Windows Host:**
```javascript
// MQTT Broker - ส่งข้อมูลไปยัง Production Server VM
host: '172.16.10.10'
port: 1883
username: 'mqtt_user'
password: 'mqtt_password'
...
#### 10.2.6 Development Mode Service Locations

**เมื่อรัน Development Mode บน Windows 10 (D:\Skill-PWS\):**

| Service | Location | Address | Port | Protocol |
|---------|----------|---------|------|----------|
| **Backend API** | Windows Host | `localhost` | 3000 | HTTP |
| **Frontend App** | Windows Host | `localhost` | 3001 | HTTP |
| **Database** | Production Server VM | `172.16.10.10` | 3306 | MySQL |
| **MQTT Broker** | Production Server VM | `172.16.10.10` | 1883 | MQTT |
| **Node-RED** | Production Server VM | `172.16.10.10` | 1880 | HTTP |
| **phpMyAdmin** | Production Server VM | `172.16.10.10` | 8080 | HTTP |

**การเชื่อมต่อ:**
1. **Frontend → Backend**: `http://localhost:3000/api`
2. **Backend → Database**: `172.16.10.10:3306`
3. **ESP32 Simulator → MQTT**: `172.16.10.10:1883`
4. **Node-RED → Database**: `172.16.10.10:3306` (ภายใน Docker Network)

**คำสั่งเริ่มต้น Development Mode:**

```bash
# 1. ตรวจสอบว่า Production Server VM ทำงานอยู่
ping 172.16.10.10

# 2. ตรวจสอบว่า Database, MQTT, Node-RED ทำงานอยู่
ssh it@172.16.10.10 "docker ps"

# 3. เริ่ม Backend (Terminal 1)
cd D:\Skill-PWS\backend
npm run dev

# 4. เริ่ม Frontend (Terminal 2)
cd D:\Skill-PWS\frontend
npm run dev

# 5. เริ่ม ESP32 Simulator (Terminal 3 - Optional)
cd D:\Skill-PWS\esp32-simulator
npm start
```
...
```bash
# ทดสอบ Database Connection จาก Windows Host
# (ต้องติดตั้ง MySQL Client หรือใช้ MySQL Workbench)
mysql -h 172.16.10.10 -u iot_user -piot_password_123 iot_db
